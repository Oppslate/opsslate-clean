...
assert.ok(authContext.includes("signupMut({"), "signup should use the Suite-owned account mutation");
assert.ok(authContext.includes("password,"), "signup should send the chosen password to the Suite-owned account mutation");
assert.ok(authContext.indexOf("await loginMut({ email: loginEmail, password })") < authContext.indexOf("fetch(`${AUTH_URL}/api/auth/login`"), "login should try the normalized Suite password before shared auth");
