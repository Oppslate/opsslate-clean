...
const typicalSection = { type: "object", additionalProperties: false, required: ["name", "stationStart", "stationEnd", "laneWidthLeft", "laneWidthRight", "shoulderWidthLeft", "shoulderWidthRight", "crossSlopeLeftPercent", "crossSlopeRightPercent"], properties: { name: { type: "string", maxLength: 200 }, stationStart: { type: "number" }, stationEnd: { type: "number" }, laneWidthLeft: nullableNumber, laneWidthRight: nullableNumber, shoulderWidthLeft: nullableNumber, shoulderWidthRight: nullableNumber, crossSlopeLeftPercent: nullableNumber, crossSlopeRightPercent: nullableNumber } } as const;
  ...
  schema: {
    ...
    properties: {
      records: { type: "array", maxItems: 2000, items: {
        ...
        required: ["physicalPageNumber", "viewKey", "geometryType", "authority", "alignmentName", "sourceLocator", "verticalDatum", "horizontalPoints", "horizontalSegments", "stationEquations", "verticalPoints", "typicalSections", "crossSectionPoints", "invertPoints", "materialLayers", "units", "confidence", "unresolvedIssues"],
        properties: {
          ...
          typicalSections: { type: "array", maxItems: 2000, items: typicalSection },
...
3. Cross-section and typical-section offsets, elevations, lane/shoulder widths and cross slopes.
   For typicalSections, store cross slope as signed vertical rise while moving
   outward from centerline: a lane falling 2 percent away from centerline is
   -2 on either side. Use the printed slope arrow/direction to establish sign.
   If outward direction is not explicit, return null and record the ambiguity.
