...
type Context = { job: Doc<"heliosCivilGeometryJobs">; run: Doc<"heliosCivilGeometryRuns">; project: Doc<"heliosProjects">; document: Doc<"heliosDocuments">; pages: Array<{ physicalPageNumber: number; sheetNumber: string; title: string; views: Array<{ viewKey: string; viewType: string; label: string }> }> };
...
export const startGeometryDocument = internalAction({ args: { jobId: v.id("heliosCivilGeometryJobs") }, handler: async (ctx, args): Promise<null> => {
  ...
  try {
    openai = client(); const blob = await ctx.storage.get(context.document.storageId); if (!blob) throw new Error("Stored plan PDF is unavailable.");
    ...
    const model = modelName(); const response = await openai.responses.create({ model, background: true, store: true, safety_identifier: `helios_${String(context.job.companyId)}`.slice(0, 64), metadata: { helios_geometry_job: String(context.job._id), helios_document: String(context.document._id) }, instructions: HELIOS_CIVIL_GEOMETRY_PROMPT, input: [{ role: "user", content: [{ type: "input_text", text: `Reconstruct explicit civil geometry from plan PDF "${context.document.fileName}" for package revision ${context.run.packageRevision}. Use this verified Foundation 4C page/view inventory to return the exact viewKey: ${JSON.stringify(context.pages).slice(0, 40_000)}` }, { type: "input_file", file_id: fileId, detail: "high" }] }], reasoning: { effort: "high" }, text: { format: heliosCivilGeometryFormat, verbosity: "medium" }, max_output_tokens: 100_000 }); responseId = response.id;
...
export const pollGeometryDocument = internalAction({ args: { jobId: v.id("heliosCivilGeometryJobs") }, handler: async (ctx, args): Promise<null> => {
  const context = await ctx.runQuery(loadReference, args); if (!context || context.job.status !== "analyzing" || !context.job.openaiFileId || !context.job.openaiResponseId) return null;
