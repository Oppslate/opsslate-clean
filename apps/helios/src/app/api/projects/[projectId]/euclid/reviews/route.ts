...
export async function POST(request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    ...
    const data = await callHeliosGateway("/helios/v1/euclid/reviews", principal, { projectId, input });
    return apiJson({ data }, 201);
  } catch (error) {
    return apiJson({ error: error instanceof Error ? error.message : "Euclid review decision could not be saved." }, 400);
  }
}
