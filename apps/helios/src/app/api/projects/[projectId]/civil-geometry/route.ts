...
export async function POST(request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    ...
    const data = await callHeliosGateway("/helios/v1/civil-geometry/review", principal, { projectId, input });
    return apiJson({ data }, 202);
  } catch (error) {
    return apiJson({ error: error instanceof Error ? error.message : "Civil geometry action could not be saved." }, 400);
  }
}
